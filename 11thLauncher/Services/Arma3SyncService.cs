...
namespace _11thLauncher.Services
{
    public class Arma3SyncService : AbstractService, IAddonSyncService
    {
        ...
        {
            string arma3SyncPath = string.Empty;
        ...
        private string SearchRegistryInstallations(RegistryKey key)
        {
            ...
            foreach (string subKeyName in key.GetSubKeyNames())
            {
                using (RegistryKey subkey = _registryAccessor.OpenSubKey(key, subKeyName))
                {
                    ...
                    var displayName = (string)_registryAccessor.GetKeyValue(subkey, ApplicationConfig.Arma3SyncRegDisplayNameEntry, string.Empty);
                    if (!displayName.StartsWith(ApplicationConfig.Arma3SyncRegDisplayNameValue)) continue;
                    arma3SyncPath = (string)_registryAccessor.GetKeyValue(subkey, ApplicationConfig.Arma3SyncRegLocationEntry, string.Empty);
                    break;
                }
            }
